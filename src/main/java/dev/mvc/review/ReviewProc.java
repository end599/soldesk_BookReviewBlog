...
public class ReviewProc implements ReviewProcInter {
  ...
  @Autowired // ReviewDAOInter interface를 구현한 클래스의 객체를 만들어 자동으로 할당해라.
  private ReviewDAOInter reviewDAO;

  @Override // 추상 메소드를 구현했음.
  public int create(ReviewVO reviewVO) {
    // -------------------------------------------------------------------
    String passwd = reviewVO.getPasswd();
    String passwd_encoded = this.security.aesEncode(passwd);
    reviewVO.setPasswd(passwd_encoded);
    // -------------------------------------------------------------------
  ...
  public ArrayList<ReviewVO> list_by_bookno(int bookno) {
    ArrayList<ReviewVO> list = this.reviewDAO.list_by_bookno(bookno);
    return list;
  }
  ...
  public ReviewVO read(int userno) {
    ReviewVO reviewVO = this.reviewDAO.read(userno);
    return reviewVO;
  }

  @Override
  public int map(HashMap<String, Object> map) {
    int cnt = this.reviewDAO.map(map);
    return cnt;
  }

  @Override
  public int youtube(HashMap<String, Object> map) {
    int cnt = this.reviewDAO.youtube(map);
    return cnt;
  }

  @Override
  public ArrayList<ReviewVO> list_by_bookno_search(HashMap<String, Object> hashMap) {
    ArrayList<ReviewVO> list = this.reviewDAO.list_by_bookno_search(hashMap);
    return list;
  }

  @Override
  public int list_by_bookno_search_count(HashMap<String, Object> hashMap) {
    int cnt = this.reviewDAO.list_by_bookno_search_count(hashMap);
    return cnt;
  }
  ...
  public ArrayList<ReviewVO> list_by_bookno_search_paging(HashMap<String, Object> map) {
     ...
     */
    int begin_of_page = ((int)map.get("now_page") - 1) * Review.RECORD_PER_PAGE;
    ...
    ArrayList<ReviewVO> list = this.reviewDAO.list_by_bookno_search_paging(map);
...
//  
//  <DIV id='paging'>
//    <span class='span_box_1'><A href='list_by_bookno.do?word=&now_page=1&bookno=4'>1</A></span>
//  <span class='span_box_2'>2</span>
//  <span class='span_box_1'><A href='list_by_bookno.do?word=&now_page=3&bookno=4'>3</A></span>
//    </DIV>
//  </DIV> 
//  <!-- 페이지 목록 출력 부분 종료 -->
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
  ...
  public int password_check(HashMap<String, Object> map) {
    ...
    int cnt = this.reviewDAO.password_check(map);
    return cnt;
  }
  ...
  public int delete(int userno) {
    int cnt = this.reviewDAO.delete(userno);
    return cnt;
  }

  @Override
  public int count_by_bookno(int bookno) {
    int cnt = this.reviewDAO.count_by_bookno(bookno);
    return cnt;
  }

  @Override
  public int delete_by_bookno(int bookno) {
    int cnt = this.reviewDAO.delete_by_bookno(bookno);
    return cnt;
  }

  @Override
  public int count_by_userno(int userno) {
    int cnt = this.reviewDAO.count_by_userno(userno);
    return cnt;
  }

  @Override
  public int delete_by_userno(int userno) {
    int cnt = this.reviewDAO.delete_by_userno(userno);
    return cnt;
  }

  @Override
  public int increaseReplycnt(int userno) {
    int count = reviewDAO.increaseReplycnt(userno);
    return count;
  }

  @Override
  public int decreaseReplycnt(int userno) {
    int count = reviewDAO.decreaseReplycnt(userno);
    return count;
  }
